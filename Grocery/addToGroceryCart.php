...
include "../includes/newnav.php"; 
 ...
 include "../includes/odCart.php";
  ...
  if($product_review_id == 10)
  {$query = "SELECT * FROM canned_food where product_id = {$product_id}";
  $image_src = "CANNED & PACKAGED FOODS/";
  }
  else if($product_review_id == 11)
  {$query = "SELECT * FROM cooking_essentials where product_id = {$product_id}";
  $image_src = "Cooking Essentials/";
  }
  else if($product_review_id == 12)
  {$query = "SELECT * FROM beverage where product_id = {$product_id}";
  $image_src = "Tea, Coffee & Beverages/";
  }
